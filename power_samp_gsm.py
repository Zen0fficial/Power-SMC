"""Benchmark script for GSM8K using naive, standard, MCMC, and SMC sampling."""
...
def extract_gsm8k_answer(answer_str: str) -> str:
    """Extract the final numeric/text answer from GSM8K answer format."""
...
def track_peak_memory(func, *args, **kwargs):
    """Run a callable and print peak CUDA memory usage in GB."""
    torch.cuda.reset_peak_memory_stats()
    torch.cuda.synchronize()
    result = func(*args, **kwargs)
    torch.cuda.synchronize()
    peak = torch.cuda.max_memory_allocated() / (1024**3)  # GB
    print(f"{func.__name__}: peak GPU memory = {peak:.2f} GB")
    return result


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run power-sampling comparisons on the GSM8K test split."
    )
    parser.add_argument(
    ...
    parser.add_argument("--dataset", action="store", default="GSM8K", type=str)
    ...
    hf_model = transformers.AutoModelForCausalLM.from_pretrained(
        model_str, torch_dtype="auto", device_map="cuda:0", trust_remote_code=False
    ).to(device)
    ...
    # Main benchmark loop.
    for data in tqdm(dataset, desc="Benchmark on GSM8K"):
        question = data["question"]
        answer = extract_gsm8k_answer(data["answer"])
        count += 1
        ...
        cfg = SMCSamplingConfig(
            ...
            ess_threshold=0.5,
            temperature=temp,  # proposal = base for clean baseline; later you can set !=1 and keep correction term
            block_size=64,
